...
class UniqueImages {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < images.length; i++){
        	String file = images[i].filename.substring(0, images[i].filename.indexOf("."));
        	if(li.isEmpty() || !li.contains(file)){
        		set.add(images[i]);
        		li.add(images[i].filename.substring(0, images[i].filename.indexOf(".")));
        	}
        }
